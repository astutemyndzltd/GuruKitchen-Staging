    ...
    public function apply($model, RepositoryInterface $repository)
    {
        if ($this->request->has(['myLon', 'myLat', 'areaLon', 'areaLat'])) {
            ...
            /*return $model->select(DB::raw("SQRT(
                ...
                return $model->select(DB::raw("(get_distance(latitude, longitude, $myLat, $myLon) / 1000) AS distance, (get_distance(latitude, longitude, $myLat, $myLon) / 1000) AS area"), "restaurants.*")
                ->orderBy('closed')
                ->orderBy('area');
        } else {
